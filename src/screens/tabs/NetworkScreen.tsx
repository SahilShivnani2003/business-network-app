...
import { leadRequirementAPI } from '../../service/apis/leadsRequirementService';
...
const LeadRequirementScreen: React.FC<Props> = ({ navigation }) => {
    const [loading, setLoading] = useState<boolean>(false);
    const [search, setSearch] = useState('');
    const [selectedIndustry, setSelectedIndustry] = useState('All');
    const [selectedCity, setSelectedCity] = useState('All Cities');
    const [allLeadRequirement, setAllLeadRequirement] = useState<any[]>([]);
    const [myLeadRequiremtn, setMyLeadRequirement] = useState<any[]>([]);
    ...
    const filtered = MOCK_MEMBERS.filter(
        m =>
            ...
            (selectedIndustry === 'All' ||
                m.industry.toLowerCase().includes(selectedIndustry.toLowerCase())),
    );

    const fetchMyLeadRequirement = async () => {
        try {
            setLoading(true);

            const response = await leadRequirementAPI.myRequirements();

            if (response.data?.success) {
                setMyLeadRequirement(response.data?.data);
            }
        } catch (error: any) {
        } finally {
            setLoading(false);
    ...
    const fetchAllLeadRequirement = async () => {
        try {
            setLoading(true);

            const response = await leadRequirementAPI.getAllActive();

            if (response.data?.success) {
                setAllLeadRequirement(response.data?.data);
            }
        } catch (error: any) {
            console.error('FETCH ALL LEAD REQUIREMENT ERROR : ', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        fetchAllLeadRequirement();
        fetchMyLeadRequirement();
    }, []);
    return (
            ...
            >
                {INDUSTRIES.map(ind => (
                    <TouchableOpacity
                        ...
                        style={[
                            styles.filterChip,
                            selectedIndustry === ind && styles.filterChipActive,
                        ]}
                    ...
                    >
                        <Text
                            style={[
                                ...
                                selectedIndustry === ind && styles.filterChipTextActive,
                            ]}
                        >
                            {ind}
                        </Text>
...
export default LeadRequirementScreen;
